(() => {
  ...
  style.textContent = `
    .dimension-analytics-panel { margin-bottom: 16px; padding: 21px 22px 16px; }
    .dimension-analytics-heading { display:flex; align-items:flex-start; justify-content:space-between; gap:18px; margin-bottom:16px; }
    .dimension-tabs { display:inline-flex; flex-wrap:wrap; gap:5px; padding:4px; border:1px solid var(--line); border-radius:12px; background:var(--panel-soft); }
    .dimension-tabs button { min-height:32px; padding:7px 11px; border:0; border-radius:9px; background:transparent; color:var(--muted); font-size:.73rem; font-weight:800; white-space:nowrap; }
    .dimension-tabs button.is-active { background:var(--panel); color:var(--text); box-shadow:0 2px 8px rgba(12,20,44,.08); }
    .dimension-summary-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:11px; margin-bottom:17px; }
    .dimension-summary-card { min-width:0; padding:15px; border:1px solid var(--line); border-radius:14px; background:var(--panel-soft); }
    .dimension-summary-card span { display:block; color:var(--muted); font-size:.66rem; font-weight:820; letter-spacing:.08em; text-transform:uppercase; }
    .dimension-summary-card strong { display:block; margin-top:8px; overflow:hidden; font-size:1rem; line-height:1.25; text-overflow:ellipsis; white-space:nowrap; }
    .dimension-summary-card small { display:block; margin-top:5px; color:var(--muted); font-size:.72rem; line-height:1.4; }
    .dimension-table { min-width:760px; }
    .dimension-table th,.dimension-table td { padding:12px 13px; }
    .dimension-segment-cell strong { display:block; }
    .dimension-segment-cell small { display:block; margin-top:4px; color:var(--muted); font-size:.69rem; }
    .dimension-volume-track { width:92px; height:6px; margin-top:6px; overflow:hidden; border-radius:999px; background:var(--panel-soft); }
    .dimension-volume-fill { height:100%; border-radius:inherit; background:linear-gradient(90deg,var(--blue),var(--purple)); }
    .dimension-sample { display:inline-flex; align-items:center; min-height:24px; padding:4px 8px; border-radius:999px; background:var(--panel-soft); color:var(--muted); font-size:.66rem; font-weight:800; white-space:nowrap; }
    ...
    .dimension-empty td { padding:28px 16px; color:var(--muted); text-align:center; }
    .dimension-footnote { margin:12px 0 0; color:var(--muted); font-size:.71rem; line-height:1.45; }
    @media(max-width:900px){.dimension-summary-grid{grid-template-columns:1fr}.dimension-analytics-heading{flex-direction:column}}
    @media(max-width:760px){.dimension-analytics-panel{padding:17px}.dimension-tabs{width:100%;display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.dimension-tabs button{width:100%}}
  `;
    ...
    panel.innerHTML = `
      <div class="dimension-analytics-heading">
        <div>
          ...
          <p class="panel-copy">Compare recorded results by market, bet timing, bookmaker, and strategy tags.</p>
        </div>
      ...
      <div class="table-wrap">
        <table class="dimension-table">
          <thead><tr><th id="dimensionColumnHeading">Market</th><th>Bets</th><th>Win rate</th><th>Avg odds</th><th>Stake</th><th>P/L</th><th>ROI</th><th>Sample</th></tr></thead>
  ...
  function dimensionLabels(bet, dimension) {
    ...
    if (dimension === "timing") return [meta.timing === "live" ? "Live" : meta.timing === "prematch" ? "Pre-match" : DIMENSIONS.timing.empty];
  ...
  function groupItems(items, dimension) {
    ...
    items.forEach((bet) => {
      dimensionLabels(bet, dimension).forEach((label) => {
        ...
        const row = groups.get(key) || { label, bets: 0, wins: 0, stake: 0, pl: 0, oddsTotal: 0 };
        row.bets += 1;
  ...
  function renderDimensionAnalytics() {
    ...
    if (!body) return;
    if (!rows.length) {
      body.innerHTML = `<tr class="dimension-empty"><td colspan="8">No settled bets in this period.</td></tr>`;
      return;
    }
    ...
    body.innerHTML = rows.map((row) => {
      ...
      return `<tr>
        <td class="dimension-segment-cell"><strong>${escapeHtml(row.label)}</strong><small>${row.bets} settled bet${row.bets === 1 ? "" : "s"}</small></td>
